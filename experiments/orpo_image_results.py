...
filenames = {
    f"Feedback-Aware-Questions/experiments/tasa_fairytaleqa_test_questions_feedback_aware_orpo_smart_duplicates.json": "Feedback-Aware ORPO",
    f"Feedback-Aware-Questions/experiments/tasa_fairytaleqa_test_questions_feedback_aware_sft_smart_duplicates.json": "Feedback-Aware SFT",
    f"Feedback-Aware-Questions/experiments/tasa_fairytaleqa_test_questions_baseline_single_answer_sft_smart_duplicates.json": "Single Selection SFT",
    f"Feedback-Aware-Questions/experiments/tasa_fairytaleqa_test_questions_baseline_single_answer_orpo_smart_duplicates.json": "Single Selection ORPO",
}
...
for filename in filenames:
    ...
    for data in dataset:
        ...
        for i in range(1, 26):
            precision_at_k_dict[f"{i}"] = len([1 for x in data[:i] if x['label'] == 'GOOD']) / i
    ...
    for i in range(1, 26):
        mean_precision_at_k_dict[i] = sum([x[f"{i}"] for x in precision_at_k]) / len(precision_at_k)
...
fig_precision.write_image("Feedback-Aware-Questions/experiments/precision_at_k_orpo.png")
